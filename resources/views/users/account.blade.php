		...
		<div class="row">
			...
			<div class="col-sm-7 col-sm-offset-1">
				<div class="login-form">
					<h2>Actualizar Sus Datos Personales</h2>
					<form id="accountForm" name="accountForm" action="{{ url('/account') }}" method="POST">{{ csrf_field() }}
						...
						<input value="{{ $userDetails->pincode }}" style="margin-top: 10px;" id="pincode" name="pincode" type="text" placeholder="Codigo Postal"/>
			...
			<div class="col-sm-6">
				<div class="signup-form">
					<h2>Puede Actualizar Tu Clave!!</h2>
					<form id="passwordForm" name="passwordForm" action="{{ url('/update-user-pwd') }}" method="POST">{{ @csrf_field() }}
						<input type="password" name="current_pwd" id="current_pwd" placeholder="Clave Actual">
						<span id="chkPwd"></span>
						<input type="password" name="new_pwd" id="new_pwd" placeholder="Nueva Clave">
						<input type="password" name="confirm_pwd" id="confirm_pwd" placeholder="Confirmar Nueva Clave">
						<button type="submit" class="btn btn-default">Actualizar</button>
					</form>
				</div>
			</div>
